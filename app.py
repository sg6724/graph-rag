...
def render_metrics():
    ...
    with metrics_box:
        c1, c2 = st.columns(2)
        c1.metric("Questions", s["queries"], help="All questions this session, in every pipeline")
        c2.metric("Cache hit rate", f"{s['hit_rate']:.0%}", f"{s['hits']}/{s['cache_queries']}",
                  help="Only questions asked in 'GraphRAG + semantic cache' mode can hit the cache")
        c1.metric("LLM calls used", s["llm_calls"])
        c2.metric("LLM calls saved", s["llm_calls_saved"])
        c1.metric("Avg cache-hit time", f"{s['avg_hit_ms']:.0f} ms")
        c2.metric("Avg LLM-answer time", f"{s['avg_answer_ms'] / 1000:.1f} s")
        ...
        served = engine.router.served
        st.caption("Answers generated by: " + (", ".join(f"{k} ×{v}" for k, v in served.items())
                                               if served else "no LLM calls yet this session"))
