...
def test_demo_walkthrough(monkeypatch):
    ...
    # plain GraphRAG questions count in the metrics too, and the model that answered is shown
    ask(at, "Does amiodarone change digoxin levels?", "graphrag")
    questions = [m for m in at.sidebar.metric if m.label == "Questions"][0]
    assert int(questions.value) >= 5, questions.value
    captions = [c.value for c in at.sidebar.caption]
    assert any("Answers generated by:" in c and "×" in c for c in captions), captions
